...
def connect_teradata(env, connector):
    ...
    if connector == "pyodbc":
        conn = odbc.connect('DRIVER={Teradata};'+f"DBCNAME={env_n};DSN={env_dsn};UID={usr};PWD={passw};QUIETMODE=YES",autocommit=True)
        return(conn)

    elif connector == "teradata":
        udaExec = teradata.UdaExec(appName="Anomaly Detection", odbcLibPath=R"C:\Users\Mark\AppData\Local\Programs\Python\Python38\Lib\site-packages\pyodbc-4.0.30-py3.8-win-amd64.egg\pyodbc.cp38-win_amd64.pyd", logConsole=False)
